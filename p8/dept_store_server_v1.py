...
def update_order(order_id):
    ...
    # validate cust_name and locate cust_id
    if cust_name_new:
        cur.execute("select cust_id from customer where cust_name = '" + cust_name_new + "'")
        try: cust_id_new = cur.fetchone()[0]
        except: return "Error updating order_id {}. Updated customer name not in system </br> Return to <a href = \"/edit_order/{}\">View/Edit Page</a>".format(order_id, order_id)
        else: update_cust = True
    else:
        cur.execute("select cust_id from orders natural join cc_belong where order_id = {}".format(order_id))
        cust_id_new = cur.fetchone()[0] # keep new cust_id the same as the old cust_id
        update_cust = False
    # validate card_number and locate card_id
    if card_number_new:
        cur.execute("select card_id from credit_card where card_number = '" + card_number_new + "'")
        try: card_id_new = cur.fetchone()[0]
        except: return "Error updating order_id {}. Updated credit card not in system </br> Return to <a href = \"/edit_order/{}\">View/Edit Page</a>".format(order_id, order_id)
        else: update_card = True
    else:
        cur.execute("select card_id from orders natural join cc_belong where order_id = {}".format(order_id))
        card_id_new = cur.fetchone()[0] # keep new card_id the same as the old card_id
        update_card = False

    # validate cust_id_new, card_id_new combination in cc_belong table and locate cc_belong_id
    if update_card or update_cust:
        # try locate the new cust_id, card_id combination
        cur.execute("select cc_belong_id from cc_belong where cust_id = {} and card_id = {}".format(cust_id_new, card_id_new))
        try: cc_belong_id_new = cur.fetchone()[0]
        except:
            # if the new combination is not in the cc_belong_id, then see if the card is already assigned to another cust_id in the cc_belong table
            cur.execute("select cc_belong_id from cc_belong where card_id = {}".format(card_id_new))
            if cur.fetchone() is not None:
                return "Error updating order_id {}. Updated redit card {} does not belong to updated customer {} in the system </br> Return to <a href = \"/edit_order/{}\">View/Edit Page</a>".format(order_id, card_id_new, cust_id_new, order_id)
            else:
                # if the new combination is not in the cc_belong_id, and the card is not assigned to another cust_id, then add a new cc_belong entry
                cur.execute("select max(cc_belong_id) from cc_belong")
                cc_belong_id_new = cur.fetchone()[0]
                cur.execute("insert into cc_belong values ({0}, {1}, {2})".format(cc_belong_id_new, cust_id_new, card_id_new))
                con.commit()
        finally: update_lst.append("cc_belong_id = {}".format(cc_belong_id_new))
    # validate sa_name and locate sa_id
    if sa_name_new:
...
def show_ordermerch(order_id):
    ...
    sql += ' natural join cc_belong natural join customer natural join credit_card natural join salesperson left outer join order_merch using (order_id) left outer join merchandise using (sku) where order_id = {}'.format(order_id)
    sql += ' limit 30'
    print(sql)
