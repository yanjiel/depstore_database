...
def hello():
    ...
    html += "Blank search fields mean no search filters on those fields. (*) indicates fields that support wildcard searching <br/>"
    html += "To see all order history (limit 30), leave all fields blank and hit 'Search Order'"
    return html
...
def search():
    ...
    if not order_id and not order_date and not cust_name and not cc_last4 and not sa_name:
        sql += ' order by order_id asc limit 30'
    else:
        sql += ' where '
        if order_id:
        ...
        sql += ' order by order_id asc limit 30' # make sure the latest on top
    ...
    html += "</table>"
    html += "*{} record(s) found. Limit 30".format(num)
...
def insert_order():
    ...
    # see if cust_name owns card_number, if does we can safely insert
    # if not, see if card_number belongs to anyone else
    # if it's a new card then add to credit_card and then update cc_belong
    cur.execute("select cc_belong_id from cc_belong natural join customer natural join credit_card where cust_name = '{}' and card_number = '{}'".format(cust_name, card_number))
    print("select cc_belong_id from cc_belong natural join customer natural join credit_card where cust_name = '{}' and card_number = '{}'".format(cust_name, card_number))
    try:
        cc_belong_id = cur.fetchone()[0]
    except:
        cur.execute("select cc_belong_id from cc_belong natural join credit_card where card_number = '{}'".format(card_number))
        print("select cc_belong_id from cc_belong natural join credit_card where card_number = '{}'".format(card_number))
        if cur.fetchone() is not None: # means this is an existing card that belongs to someone else
            return "Error inserting new order. The credit card belongs to some other customer </br> Return to <a href = \"/homepage\">main page</a>"
            ...
            card_id = cur.fetchone()[0] + 1
            cur.execute("insert into credit_card values ({}, '{}', '0000 Placeholder Dr', '', 'Placeholder', 'ZZ', '00000', 'USA')".format(card_id, card_number))
            # then see if the updated customer is in the system, if no then add customer
            cur.execute("select cust_id from customer where cust_name = '{}'".format(cust_name))
            try: cust_id = cur.fetchone()[0]
            except:
                cur.execute("select max(cust_id) from customer")
                cust_id = cur.fetchone()[0] + 1
                cur.execute("insert into customer values ({}, '{}', '')".format(cust_id, cust_name))
            finally:
                # then add the new cc_belong entry
                cur.execute("select max(cc_belong_id) from cc_belong")
                cc_belong_id = cur.fetchone()[0] + 1
                cur.execute("insert into cc_belong values ({}, {}, {})".format(cc_belong_id, cust_id, card_id))
    ...
    return "New order_id {} has been inserted </br> Return to <a href = \"/homepage\">main page</a> </br> Continue to <a href = \"/edit_order/{}\">View/Edit Order</a>".format(order_id, order_id)
